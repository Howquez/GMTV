        ...
        if self.round_number <= self.session.config["num_rounds"]:
            if self.round_number == Constants.belief_elicitation_round:
                if self.session.config["belief_elicitation"]:
                    yield dPGG_Belief, dict(belief=random.randint(int(self.player.endowment/2),
                                                                        self.player.endowment)*Constants.num_others_per_group
                                                  )
            yield Submission(dPGG_Decision, # in Gächter et al, subjects first contribute around 60%, then 30%
            ...
            if self.round_number == self.session.config["num_rounds"]:
                ...
                yield dPGG_Comprehension, dict(comprehension=a)
                yield dPGG_Donation, dict(donation=random.randint(0, int(self.player.endowment/2)))