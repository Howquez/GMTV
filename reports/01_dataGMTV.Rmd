...
`r NROW(GMTVFirstRound)` participants yield `r NROW(GMTVFirstRound)` independent observations that are stored in a data table called ```GMTVFirstRound```. These observations are displayed below and can, once more, be downloaded by a click on the button. As before, the data is also saved in ```../data/processed/```.
...
GMTVRisk[,
         switching_row := ifelse(test = inconsistent == TRUE,
                                yes  = NA,
                                no   = e10 + e20 + e30 + e40 + e50 + e60 + e70 + 1)
         ]
```
