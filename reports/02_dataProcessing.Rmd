...
Eventually, this report can also be used to process real data. This only requires us to load a different data set.^[It may also require us to merge multiple data sets -- that depends on whether the lab delivers one or several data sets and remains to be discussed.]

## First round

Eventually, we'll be interested in the participants' first round's behavior, as it indicates their willingness to cooperate before they interact with one another. As a consequence, `r NROW(DT)` participants yield `r NROW(DT)` observations that shall be stored in a data table called ```replicationFirstRound```. This table carries information on a participant's ```belief``` about the sum of other group members' contributions, their actual contribution ```othersContribution```, the participant's ```ownContribution```, some fairness measure that equals the difference between the own contribution and the expected average contribution of others and a trust measure^["Generally, others can be trusted". Likert scale ranging from 0 (do not agree) to 6 (agree).] obtained from the personality test at the end of the experiment.
...
replicationFirstRound <- DT[,
                 .(participant.code,
                   ...
                   session.code,
                   groupID = dPGG.1.group.id_in_subsession,
                   ...
                   fairness = dPGG.1.player.contribution - dPGG.1.player.belief/3,
                   trust = Outro.1.player.PQ11,
                   comprehension = Outro.1.player.Comprehension)]
...
mRegex <- "participant\\.code$|session\\.code$|dPGG\\.1\\.group\\.id_in_subsession|^dPGG.1.player.id_in_group$|\\.1\\.player.belief|endowment|contribution|stock|gain$|bot_active|Comprehension"
...
```{r calcGini}
# it would make more sense to calculate the stocks' gini but gaechter et al use the endowments
var = names(subset) %>% str_subset(pattern = "player\\.endowment$")
gini = subset[,
...
noComp <- subset[Outro.1.player.Comprehension == 0,
                 groupID] %>% unique()
replication[,
     noComprehension := 0]
