...
base::load(file = "../data/processed/demoSession.rda") 
...
```{r combineData}
combined <- rbindlist(list(longDT[treatment == "Hi000"],# | treatment == "Lo000"], 
                           gaechter,
                           demoSession))

combined[treatment == "demoSession"] %>%
  # head(n = 9) %>%
  kbl() %>% 
...
The following graph displays the zero risk treatments together with @GACHTER2017's data. The original ```noPunish10``` data lies between both sets of simulations. 
